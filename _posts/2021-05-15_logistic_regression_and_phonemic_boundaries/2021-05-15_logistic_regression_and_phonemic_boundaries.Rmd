...
ggplot(data.frame(x = c(-6, 6)), aes(x = x)) + 
  ...
  geom_text(data = sigmoid_text, aes(y = y, label = info, color = labs), 
            hjust = 0, show.legend = F, size = 3, family = "Times") + 
...
I won't go into more detail here, but check out XXX, XXX and XXX. 
</aside>
...
That is, the actual continuum of stimuli they hear is **exactly the same**. 
Tricky, right?
...
If you're doing the math, that is 25 participant $\times$ 13 steps $\times$ 15 item repetitions $\times$ 2 language modes, which gives us a data set with a grand total of `r 25 * 13 * 15 * 2` responses (i.e., `r 25 * 13 * 15 * 2` rows in the dataframe). 
(Note: If you aren't interested in the whole simulation process just skip to the next section.)
...
As mentioned above, our data is binary, so we will fit a multilevel logistic regression model. 
...
We can also plot the sigmoids of individuals along with the 50% crossovers. 
Let's find the individuals with the largest and smallest differences between English and Spanish boundaries, plus 8 more at random. 
...
<!--
Morrison G (2007) Logistic regression modeling for first- and second-language perception data. In: Solé M-J, Prieto P, and Mascaró J (eds) Segmental and prosodic issues in Romance phonology. Amsterdam: John Benjamins, 219–36.
-->
